...
export const SterilizationPage = () => {
  ...
  const handleInstrumentDragToDepartment = (
    ...
    const departmentDropzoneId = `department-dropzone-${targetDepartmentId}`;

    if (instrumentToMove && targetDepartmentId) {
      // Čia atliekame patikrą: ar toks instrumentas jau yra šiame skyriuje
      const existingInstrumentInDepartment = instrumentsSelected.some(
        (item) =>
          item.departmentId === departmentDropzoneId && // Lyginame su tiksliu dropzone ID
          item.instrument.id === instrumentToMove.id
      );

      if (existingInstrumentInDepartment) {
        toast.error(
          `${instrumentToMove.instrument_name} jau yra šiame skyriuje`
        );
      } else {
        dispatch(
          addInstrumentToDepartment({
